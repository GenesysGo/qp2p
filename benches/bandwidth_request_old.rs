...
async fn main() -> Result<()> {
    ...
    let (node, mut incoming_conns, _) =
        Endpoint::new_peer((Ipv4Addr::LOCALHOST, BASE_PORT), &[], Config::default())
            .await
            .unwrap();
    println!("set up on {}\n", node.local_addr());
    ...
    let server_handle = tokio::task::spawn(async move {
        while let Some((_connection, mut incoming)) = incoming_conns.next().await {
            ...
            while let Ok(Some(msg)) = incoming.next().await {
                let m = msg.clone();
                drop(m);
    ...
    for i in 0..num_nodes {
        let (node, incoming, _) = Endpoint::new_peer(
            (Ipv4Addr::LOCALHOST, BASE_PORT + 1 + i),
            &[],
            Config::default(),
        )
        .await
        .unwrap();
        nodes.push((node, incoming));
    }

    // Connect to and spam peer
    let handles = nodes.into_iter().map(|(node, _incoming)| {
        tokio::spawn(async move {
            ...
            for _ in 0..num_loops {
                ...
                // Send message
                for _ in 0..num_msgs {
                    conn.send(msg.clone()).await.expect("benchmark");
                }
    ...
    // Calculate load
    let total_conn = num_loops as u128 * num_nodes as u128;
    let total_msgs = total_conn * num_msgs as u128;
